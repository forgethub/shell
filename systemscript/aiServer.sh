...
exec="python aiServer.py"
prog="Aiserver"
run_command="/usr/local/services/python2.7/bin/python2.7"
...
start(){
    ...
    AIRUNNING=0
    if [ -f "$mypidfile" ]; then
        AIPID=`cat "$mypidfile" 2>/dev/null`
        if [ -n "$AIPID" ] && [ -d "/proc/$AIPID" ] ; then
            AIRUNNING=1
        fi
    fi
    status -p "$mypidfile" -b "${run_command}" $prog
    if [ $AIRUNNING = 1 ] && [ $? = 0 ]; then
	# already running, do nothing
	action $"Starting $prog: " /bin/true
	ret=0
    elif [ $AIRUNNING = 1 ] && echo "$RESPONSE" | grep -q "Access denied for user"
    then
	# already running, do nothing
	action $"Starting $prog: " /bin/true
	ret=0
...
stop(){
	...
	AIPID=`cat "$mypidfile" 2>/dev/null`
	if [ -n "$AIPID" ]; then
	    /bin/kill "$AIPID" >/dev/null 2>&1
	    ret=$?
	    if [ $ret -eq 0 ]; then
		TIMEOUT="$STOPTIMEOUT"
		while [ $TIMEOUT -gt 0 ]; do
		    /bin/kill -9 "$AIPID" >/dev/null 2>&1 || break
		    sleep 1
		    let TIMEOUT=${TIMEOUT}-1
		done
		if [ $TIMEOUT -eq 0 ]; then
